...
async function doCampusLogin(username, password) {
  ...
  // detect browser for error handling
  const isFirefox = navigator.userAgent.includes('Firefox');

  // helper: fetch with XHR fallback for Firefox
  ...
  try {
    ...
    try {
      ...
      const portal_text = await fetchWithFallback(portal_url);
      console.log('portal_text length:', portal_text ? portal_text.length : 0);

      if (!portal_text) {
        return { success: false, message: 'no_response_data_error' };
      }
    ...
    const challenge_text = await fetchWithFallback(challenge_url);
    if (!challenge_text) {
    ...
    const login_text = await fetchWithFallback(login_url);
    if (!login_text) {
